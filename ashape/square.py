...
class Square():
    ...
    def new(self, args):
        ...
        self.square_in_char = args.square_in_char[0]
    ...
    def get_square_size(self):
        ...
        # width,heightどちらかが指定されている場合
            ...
            i = 1
            while 1:
                if (set_value - 1) * 2 + (i - 1) * 2 >= len(str_list):
                    break
                i += 1
    ...
    # @brief:
    #    四角形の枠を出力するための数字配列の生成関数
    def get_square_num_array(self, max_x, max_y):
        ...
        # 出力範囲の数字リストを作成する
        # num_range = range((max_x - 1) * 2 + (max_y - 1) * 2)
        num_range = range((max_x - 1) * 2 + (max_y - 1) * 2)
